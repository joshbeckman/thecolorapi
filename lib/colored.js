...
exports.colorMe = colorMe = function (arg){
  ...
  function fromRGB(rgb){
    if(!rgb.is_fraction){
      ...
      return fromHex(colored.schemer.stringlify.hex(rgb));
    }
  }
  function fromHex(hex){
    ...
    c.image.bare = "http://placehold.it/300x300.png/" + c.hex.clean;
    c.image.named = "http://placehold.it/300x300.png/" + c.hex.clean + "&text=" + c.name.value.split(' ').join('+');
    return c;
  }
};
    ...
    "generate": function(mode, colors, color) {
      ...
      for (groupIndex = _l = 0, _len3 = scheme.length; _l < _len3; groupIndex = ++_l) {
        ...
        for (colorCount = _m = 1, _ref = counts[groupIndex]; 1 <= _ref ? _m <= _ref : _m >= _ref; colorCount = 1 <= _ref ? ++_m : --_m) {
          ...
          colors[colorIndex] = colorMe({hsl: hsl});
          colorIndex += 1;
        }
      }
      return colors;
